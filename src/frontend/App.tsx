...
export default function App({ analyzeProject, inspectConnected }: { analyzeProject?: (targetPath: string, runtimeArtifactPath?: string) => Promise<LocalProjectAnalysis>; inspectConnected?: (input: ConnectedUiRequest) => Promise<ConnectedLocalProjectAnalysis> }) {
  ...
  return <div className="min-h-screen bg-[#031427] text-[#d3e4fe]">
    ...
    <main className="mx-auto max-w-7xl space-y-6 px-6 py-8">
      <section className="glass-panel rounded-xl p-5"><form onSubmit={submit}><label htmlFor="project-path" className="text-sm font-bold text-white">Local project directory</label><p className="mt-1 text-xs text-[#8d90a0]">The loopback-only process reads this directory locally. It does not clone, fetch, upload source, or use remote provider APIs.</p><div className="mt-4 grid gap-3 sm:grid-cols-[1fr_1fr_auto]"><div className="relative"><FolderSearch className="absolute left-3 top-2.5 h-4 w-4 text-[#8d90a0]" /><input id="project-path" value={targetPath} onChange={event => setTargetPath(event.target.value)} disabled={loading} placeholder="/absolute/path/to/project" className="w-full rounded-md border border-white/15 bg-[#031427] py-2.5 pl-10 pr-3 font-mono text-xs text-white outline-none focus:border-primary" /></div><input aria-label="Runtime artifact" value={runtimeArtifactPath} onChange={event => setRuntimeArtifactPath(event.target.value)} disabled={loading} placeholder="Sanitized runtime JSONL (optional)" className="rounded-md border border-white/15 bg-[#031427] px-3 py-2.5 font-mono text-xs text-white" /><button disabled={loading} className="rounded-md bg-primary px-5 py-2.5 text-xs font-bold text-white hover:bg-primary-hover disabled:opacity-50">{loading ? 'Analyzing…' : 'Analyze with tAIdyup'}</button></div><p className="mt-2 text-xs text-[#8d90a0]">Runtime evidence is imported only when you explicitly provide a local sanitized JSONL artifact. No monitoring or provider connection is started.</p></form></section>
