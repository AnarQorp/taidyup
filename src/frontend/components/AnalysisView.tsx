...
export function ClaimDetail({ claim, result, onClose }: { claim: Claim; result: LocalProjectAnalysis | ConnectedLocalProjectAnalysis; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4" role="dialog" aria-label="Claim detail">
    <div className="glass-panel max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-xl p-6">
      ...
      <section className="mt-4 rounded-lg border border-white/10 bg-[#031427] p-4"><h3 className="text-xs font-bold uppercase tracking-wider text-white">What tAIdyup doesn't know</h3><ul className="mt-3 grid gap-2 text-sm text-[#8d90a0] sm:grid-cols-2">{(!claim.runtimeAssessment || claim.runtimeAssessment.observationState === 'NO_OBSERVATION') && <li>? Whether this capability was executed.</li>}<li>? Whether the action was authorized.</li><li>? Whether the downstream effect occurred.</li><li>? Whether the result was correct.</li><li>? Whether all executions were observed.</li><li>? Safety and compliance are not established.</li></ul></section>
