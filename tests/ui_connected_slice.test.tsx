...
async function run() {
  ...
  try {
    ...
    const t1Html = renderToStaticMarkup(<AnalysisView state={{ status: 'success', result: t1.body }} />); assert.match(t1Html, /Latest inspected snapshot/); assert.match(t1Html, /n8n:synthetic-ui:/); assert.match(t1Html, /CONNECTED/); assert.match(t1Html, /Runtime/); assert.match(t1Html, /Unavailable/); assert.match(t1Html, /data-capability-action="SEND"/); assert.match(t1Html, />Gmail</); assert.match(t1Html, /Capability evidence layers/); assert.doesNotMatch(t1Html.match(/data-capability-action="SEND"[\s\S]*?<\/article>/)?.[0] || '', /\{&quot;namespace&quot;|taidyup-ui-connected-/);
    includeGmail = false;
