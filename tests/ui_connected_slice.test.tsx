...
async function run() {
  ...
  try {
    ...
    const t1Html = renderToStaticMarkup(<AnalysisView state={{ status: 'success', result: t1.body }} />); assert.match(t1Html, /Latest inspected snapshot/); assert.match(t1Html, /n8n:synthetic-ui:/); assert.match(t1Html, /CONNECTED/); assert.match(t1Html, /Runtime/); assert.match(t1Html, /No runtime evidence available/); assert.match(t1Html, /data-capability-action="SEND"/); assert.match(t1Html, />Gmail</); assert.match(t1Html, /Capability evidence layers/); assert.doesNotMatch(t1Html.match(/data-capability-action="SEND"[\s\S]*?<\/article>/)?.[0] || '', /\{&quot;namespace&quot;|taidyup-ui-connected-/);
    includeGmail = false;
    ...
    const t2Html = renderToStaticMarkup(<AnalysisView state={{ status: 'success', result: t2.body }} />); assert.match(t2Html, /data-drift-outcome="CURRENT_STATE_DRIFT"/); assert.match(t2Html, /Current connected configuration changed/); assert.equal((t2Html.match(/data-drift-outcome="CURRENT_STATE_DRIFT"/g) || []).length, 1); assert.match(t2Html, /View change \/ Why\?/);
    const detail = renderToStaticMarkup(<ClaimDetail claim={claim} result={t2.body} onClose={() => {}} />); assert.match(detail, /ABSENCE_OBSERVED/); assert.match(detail, /CURRENT_STATE_DRIFT/); assert.match(detail, /Why this result/); assert.match(detail, /What changed/); assert.match(detail, /present/); assert.match(detail, /absent/); assert.match(detail, /Technical evidence/); assert.match(detail, /Full provenance/); assert.match(detail, /View raw resource/); assert.match(detail, /Whether execution activity occurred outside the available runtime evidence/); assert.match(detail, /Whether the action was authorized/); assert.doesNotMatch(detail, /ui-secret-value|permission changed|Agent stopped|Credential removed/);
